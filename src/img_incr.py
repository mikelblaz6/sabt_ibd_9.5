...
def set_full_upd_files(args, project_tree, project_list, work_path):
	...
	temp_files = ("main.sh", "update_template.sh")
	...
	for temp_file in temp_files:
		...
		tags = [("$FW_VERSION_PAR$", args.final_release_version),
				...
				("$PRE_ACTIONS_SECTION$", pre_actions_text),
				("$POST_ACTIONS_SECTION$", post_action_text),]
	...
	os.system("cp -aRf " + constants.INSTALL_DIR + "/* " + work_path + "/update_files/system/")
...
def pack_fw(part_number, dest_file, work_path):
	...
	os.system("mv " + work_path + "/update_template.sh " + compress_dir)
	os.system("mv " + work_path + "/update_files " + compress_dir)
	os.system("tar -Jcf " + dest_file + " -C " + compress_dir + " main.sh update_template.sh update_files")
